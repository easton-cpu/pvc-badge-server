...
def cors_response(data, status=200):
    """Wrap any JSON response with CORS headers so GHL pages can reach us."""
    resp = make_response(jsonify(data), status)
    resp.headers["Access-Control-Allow-Origin"]  = "*"
    resp.headers["Access-Control-Allow-Methods"] = "POST, GET, OPTIONS"
    resp.headers["Access-Control-Allow-Headers"] = "Content-Type, X-PVC-Secret"
    return resp
...
def health():
    return cors_response({"status":"ok","service":"PVC Badge Generator"})
 
# Handle preflight OPTIONS requests from browsers (required for CORS)
@app.route("/badge", methods=["OPTIONS"])
def badge_preflight():
...
def badge_webhook():
    ...
    try:
        ...
        if not name or not email:
            return cors_response({"error":"full_name and email are required"}, 400)
        pdf=generate_badge(name,title,phone,email,photo_bytes)
        send_badge_email(email,name,pdf)
        return cors_response({"success":True,"message":f"Badge emailed to {email}"}),200
    except Exception as e:
        traceback.print_exc(); return cors_response({"error":str(e)}, 500)
